...
/* Animations */
const pageVariants = { hidden: { opacity: 0 }, show: { opacity: 1, transition: { when: "beforeChildren", staggerChildren: 0.12 } } };
const fadeUp = { hidden: { opacity: 0, y: 22 }, show: { opacity: 1, y: 0, transition: { duration: 0.55, ease: [0.22, 1, 0.36, 1] } } };
const zoomIn = { hidden: { opacity: 0, scale: 0.96 }, show: { opacity: 1, scale: 1, transition: { duration: 0.5, ease: [0.22, 1, 0.36, 1] } } };
...
function Home() {
  ...
  // Toggle kelas highlight pada link topbar yang punya data-spy-link
  useEffect(() => {
    const ACTIVE = ["border-b-2", "border-blue-600", "text-blue-700"];
    const BASE = ["border-b-2", "border-transparent"]; // supaya tinggi nav stabil

    const links = document.querySelectorAll("[data-spy-link]");
    links.forEach((el) => {
      BASE.forEach((c) => el.classList.add(c)); // pastikan ada base
  ...
  return (
    <motion.div className="flex flex-col h-screen w-full overflow-hidden" variants={pageVariants} initial="hidden" animate="show">
      ...
      <div className="flex-1 overflow-y-auto w-full scrollbar-hide">
        {/* Welcome Section */}
        <section id="home" className="scroll-mt-24 w-full bg-white-100 py-12 px-4 sm:px-6 lg:px-8">
          <motion.div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start gap-8" variants={sectionStagger} initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.2 }}>
            <motion.div className="md:w-3/3" variants={fadeUp}>
              ...
              <p className="text-xl text-gray-600 mb-8">Your Trusted Partner in Website Development - Building Digital Success, Together.</p>
              <motion.button onClick={() => setShowLoginForm(true)} className="bg-blue-600 text-white py-3 px-8 rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-colors text-lg font-medium" variants={zoomIn} whileHover={{ y: -2 }} whileTap={{ scale: 0.98 }}>
                Get Started
              </motion.button>
            </motion.div>

            <motion.div className="md:w-1/3 w-full" variants={zoomIn}>
              <img src={WelcomeImage} alt="Welcome" className="rounded-lg shadow-md object-cover w-full h-auto max-h-64 md:max-h-80" />
            </motion.div>
          </motion.div>
        </section>

        {/* About Us Section */}
        <section id="about" className="scroll-mt-24 w-full bg-gray-100 py-12 px-4 sm:px-6 lg:px-8">
          <motion.div className="max-w-7xl mx-auto flex flex-col md:flex-row gap-8" variants={sectionStagger} initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.25 }}>
            <motion.div className="md:w-2/3" variants={fadeUp}>
              ...
              <p className="text-gray-700 mb-4 leading-relaxed">Choose Bytelogic for reliable service, transparent communication, and results-driven solutions that elevate your business in the digital world.</p>
            </motion.div>

            <motion.div className="md:w-1/3 flex items-center justify-center" variants={zoomIn}>
              <img src={AboutUsImage} alt="About Us" className="rounded-lg shadow-md object-cover w-full h-auto max-h-64 md:max-h-80" />
            </motion.div>
          </motion.div>
        </section>

        {/* Services Section (horizontal + panah) */}
        <section id="services" className="scroll-mt-24 w-full bg-blue-100 py-12 px-4 sm:px-6 lg:px-8">
          <div className="max-w-7xl mx-auto">
            <motion.h1 className="text-4xl font-bold text-gray-800 mb-4 text-center" variants={fadeUp} initial="hidden" whileInView="show" viewport={{ once: true }}>
              Our Services
            </motion.h1>
            <motion.p className="text-xl text-gray-700 mb-12 text-center" variants={fadeUp} initial="hidden" whileInView="show" viewport={{ once: true }}>
            ...
            <div className="relative w-full">
              ...
              <button
                ...
                className="hidden md:flex items-center justify-center absolute left-0 top-1/2 -translate-y-1/2 z-10 h-10 w-10 rounded-full bg-white/90 shadow hover:bg-white"
              >
              ...
              <button
                ...
                onClick={() => scrollServicesBy(360)}
                className="hidden md:flex items-center justify-center absolute right-0 top-1/2 -translate-y-1/2 z-10 h-10 w-10 rounded-full bg-white/90 shadow hover:bg-white"
              >
              ...
              <div
                ref={servicesRef}
                className="flex overflow-x-auto pb-6 px-10 gap-6 snap-x snap-mandatory scroll-smooth scrollbar-hide"
              >
                {[
                  ...
                  <motion.div
                    key={idx}
                    className="snap-start flex-none w-80 md:w-96 min-h-[260px] bg-gradient-to-r from-blue-600 to-blue-800 p-6 rounded-lg shadow-md hover:shadow-lg transition-shadow"
                    variants={fadeUp}
                    initial="hidden"
                    whileInView="show"
                    ...
                    whileHover={{ y: -4 }}
                  >
                    <h2 className="text-xl font-semibold text-white mb-3 text-center">{card.title}</h2>
                    <p className="text-white/90 text-center">{card.desc}</p>
                  </motion.div>
                ))}
              </div>
            </div>
        ...
        <section id="projects" className="scroll-mt-24 w-full bg-white py-12 px-4 sm:px-6 lg:px-8">
          <div className="max-w-7xl mx-auto">
        ...
        {/* Reviews */}
        <section id="review" className="scroll-mt-24 w-full bg-blue-100 py-12 px-4 sm:px-6 lg:px-8">
          <div className="max-w-4xl mx-auto">
            <motion.h1 className="text-4xl font-bold text-gray-800 mb-2 text-center" variants={fadeUp} initial="hidden" whileInView="show" viewport={{ once: true }}>
              Reviews
            </motion.h1>
        ...
        <section id="contact" className="scroll-mt-24 w-full bg-[#3B82F6] py-16 pl-0 pr-0 overflow-hidden">
          <div className="max-w-7xl mx-auto">
            <div className="relative">
              <motion.div className="bg-white rounded-r-3xl rounded-l-none p-8 md:p-12 shadow-lg -ml-4 md:-ml-8 lg:-ml-16 xl:-ml-60" variants={zoomIn} initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.2 }}>
                      ...
                      <form className="space-y-6" onSubmit={handleContactSubmit}>
                        <div className="grid grid-cols-1 gap-6">
                          <div>
                            ...
                            <input type="email" name="email" value={contactForm.email} onChange={handleContactChange} className="w-full px-4 py-3 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="Your email" required />
                          </div>
                          <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">Phone</label>
                            <input type="tel" name="phone" value={contactForm.phone} onChange={handleContactChange} className="w-full px-4 py-3 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="Your phone" required />
                          </div>
                          <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">Message</label>
                            <input type="text" name="message" value={contactForm.message} onChange={handleContactChange} className="w-full px-4 py-3 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="Your message" required />
                          </div>
                        </div>
                        <div className="flex justify-end">
                          <button type="submit" className="bg-[#3B82F6] text-white py-3 px-8 rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-colors text-lg font-medium">
                            Send
                          </button>
                        </div>
                      </form>
            ...
            <div className="flex flex-col md:flex-row justify-between items-center">
              ...
              <div className="flex items-center space-x-6">
                ...
                <div className="flex flex-col">
                  <a href={`https://wa.me/${WHATSAPP_NUMBER}`} target="_blank" rel="noopener noreferrer" className="text-sm opacity-70 hover:opacity-100 transition-opacity"><b>+{WHATSAPP_NUMBER}</b></a>
                  <a href="mailto:hello@bytelogic.com" className="text-sm opacity-70 hover:opacity-100 transition-opacity">hello@bytelogic.com</a>
                </div>
              </div>
            </div>
          </div>
