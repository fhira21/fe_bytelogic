...
/* ===== Helpers (sama seperti On Progress) ===== */
...
function normalizeProject(p = {}) {
  ...
  return {
    ...
    client_id,
    // kalau BE sudah populate / ada client_name, pakai itu; kalau "client" string tapi bukan ObjectId, pakai stringnya
...
const ProjectCompleted = () => {
  ...
  useEffect(() => {
    const fetchCompletedProjects = async () => {
      ...
      try {
        ...
        // 1) Ambil semua clients → buat map id -> nama (sekali)
        ...
        // 2) Ambil ringkasan total project → bucket "Completed"
        ...
        const completedList = totalRes.data?.data?.statusSummary?.detail?.["Completed"] || [];

        // 3) Normalisasi + isi nama klien:
        //    a) pakai embedded/client_name dari BE kalau ada
        //    b) kalau hanya ID, pakai nama dari clientsMap
        //    c) fallback "-"
        const normalized = completedList.map((p) => {
  ...
  return (
    <div className="min-h-screen bg-white px-8 py-6">
      ...
      <h2 className="text-xl font-semibold mb-4">Status Project : Completed</h2>
