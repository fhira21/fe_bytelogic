// src/pages/klien/EditProfile.jsx
...
// Kompres & resize gambar jadi kecil (≈ ≤120KB)
...
export default function KlienEditProfile() {
  const [profile, setProfile] = useState({
    ...
    nomor_telepon: "",
    alamat: "",
    foto_profile: "", // path server (/uploads/...), dataURL, atau ""
  });
  ...
  const refetchProfile = async () => {
    ...
    try {
      ...
      const data = root.client || root; // beberapa API membungkus { client: {...} }
      ...
      console.error("GET /clients/profile error:", err?.response?.data || err);
      setErrorText(err?.response?.data?.message || "Gagal memuat profil klien.");
    } finally {
      setLoading(false);
    }
  };
  ...
  // 2) multipart ke /api/clients/profile
  ...
  // 3) JSON ke /api/clients/:id
  const tryPutJsonById = async () => {
    if (!profile._id) {
      const err = new Error("Client ID kosong untuk update by ID.");
      err.code = "NO_ID";
      throw err;
    }
    const url = `${API_BASE}/api/clients/${profile._id}`;
    const payload = buildJsonPayload();
  ...
  const onSubmit = async (e) => {
    ...
    try {
              ...
              try {
                ...
                if ([400,401,403,404,405,413,415,500].includes(st3) || !st3) {
                  ...
                  try {
                    ...
                    console.error("ALL UPDATE FAILED", {
      ...
      // beri tahu header/topbar untuk reload avatar
      ...
      alert(`Gagal memperbarui profil klien: ${typeof msg === "string" ? msg : "Periksa kembali data & endpoint."}`);
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <h1 className="text-2xl font-semibold text-gray-900 mb-8">Edit Profile (Client)</h1>
